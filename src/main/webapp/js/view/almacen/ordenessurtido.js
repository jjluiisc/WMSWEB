...
function obtenOrdenesSurtido() {
    ...
    // Todos las ordenes de surtido, PE = Pendientes, SU = Surtiendose y TS = Terminado de Surtir
    var where = "osp.status IN ('PE','SU','TS')";
...
function creaGridOrdenesSurtido() {
    ...
    $grid.jqxGrid({
        ...
        columns: [
            ...
            {text: "Pedido", datafield: "pedido", width: "11%", cellclassname: cellclass},
            {text: "Ruta", datafield: "ruta", width: "6%", cellclassname: cellclass},
            ...
            {text: "Cantidad", datafield: "cantidad", cellsalign: "right", cellsformat: "f0", width: "7%", cellclassname: cellclass},
            {text: "Surtidas", datafield: "surtidas", cellsalign: "right", cellsformat: "f0", width: "7%", cellclassname: cellclass},
...
function muestraOrdenesSurtido(response) {
    ...
    for(var indx=0; indx<response.length; indx++) {
        currentOrder.push(response[indx].flpedido);
        if (response[indx].status==="PE")
            pendientes++;
        else if (response[indx].status==="SU")
            surtiendo++;
        else if (response[indx].status==="TS")
            terminando++;
    }
...
function cancelaOrdenSurtido() {
    if (!validaPermiso("ordenesSurtidoCancelar"))
        return false;
    ...
    if (rowdata.status==="PE") {
        var cancelar = function() {
        };
        var aceptar = function() {
            cancelaPendienteOrdenSurtidoConfirmado(rowdata);
        };
        pregunta("&iquest;Realmente desea Cancelar la Orden de Surtido <b>["+rowdata.flsurtido+" "+rowdata.pedido+"]</b> ?", aceptar, cancelar);
    }
    else if (rowdata.status==="SU") {
        var cancelarII = function() {
        };
        var aceptarII = function() {
            cancelaSurtidoOrdenSurtidoConfirmado(rowdata);
        };
        pregunta("&iquest;Realmente desea Cancelar EL SURTIDO DE la Orden de Surtido <b>["+rowdata.flsurtido+" "+rowdata.pedido+"]</b>?<br>"
            +"<b>Tenga en cuenta que SI SE ESTA SURTIENDO en alguna terminal, la Orden de Surtido quedara inconsistente en la terminal.</b><br>"
...
function cancelaSurtidoOrdenSurtidoConfirmado(rowdata) {
    var data = {
        id: "CancelaSurtidoOrdenSurtidoPedido",
        compania: rowdata.compania,
        usuario: usuario.usuario,
        flsurtido: rowdata.flsurtido
    };
...
function cancelaPendienteOrdenSurtidoConfirmado(rowdata) {
    var data = {
        id: "CancelaPendienteOrdenSurtidoPedido",
        compania: rowdata.compania,
        usuario: usuario.usuario,
        flsurtido: rowdata.flsurtido
    };
