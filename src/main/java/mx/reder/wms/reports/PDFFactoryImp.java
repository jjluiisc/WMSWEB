...
public class PDFFactoryImp extends PdfPageEventHelper implements PDFFactory {
    static Logger log = Logger.getLogger(PDFFactoryImp.class.getName());
    ...
    public byte[] genera(DatabaseServices ds, ComprobanteDocument cd, TimbreFiscalDigitalDocument tfd, CartaPorteDocument cpd, PagosDocument pd,
        ...
        for (Concepto concepto : conceptos) {
            String[] tokensDescripcion = splitDescripcion(concepto.getDescripcion());
        ...
        float lineTotalHeigth = (conceptos.length * (detalleWidth * lineasPorDetalle))+lineasAdicionalesDescripcionHeigth;
        lineTotalHeigth += (conceptos.length * 5f);
        ...
        for (Concepto concepto : conceptos) {
            printDetalle(comprobante, timbreFiscal, cartaPorte, concepto);
        ...
        if (paginaNumero!=totalPaginas) {
            ...
            log.info("lineas = ["+conceptos.length+"] paginacion = ["+paginaNumero+" / "+totalPaginas+"]");
    ...
    public void printHeader(Comprobante comprobante, TimbreFiscalDigital timbreFiscal) throws Exception {
        ...
        writeElement(comprobanteParagraph, 300f, 805f, 580f, 745f);

        // Divisor
        drawRectangleStroke(15f, 740f, 580f, 739f, clearGray);

        // Receptor
        drawRectangleStroke(15f, 720f, 300f, 735f, clearGray, clearGray);
        printHeaderTitle("Cliente", headerBold, 15f, 715f, 300f, 725f, Element.ALIGN_CENTER);
        ...
        String receptorNombre = nullText(receptor.getNombre());
        String receptorRfc = receptor.getRfc();
        ...
        writeElement(clienteParagraph, 15f, 720f, 300f, 665f);
    ...
    public void printCartaPorte(Comprobante comprobante, TimbreFiscalDigital timbreFiscal, CartaPorte cartaPorte) throws Exception {
        ...
        for (CartaPorte.Ubicaciones.Ubicacion ubicacion : ubicaciones) {
            ...
            //EntidadFederativaDAO entidadFederativaDAO = new EntidadFederativaDAO(ubicacion.getDomicilio().getEstado());
            ...
            String direccion2 = "Colonia: "
                   ...
                   +ubicacion.getDomicilio().getMunicipio()+" Estado: "
                   +ubicacion.getDomicilio().getEstado();
            ...
            String direccion3 = "Localidad: "
                   +ubicacion.getDomicilio().getLocalidad()+" Pais: "
    ...
    public void printDetalle(Comprobante comprobante, TimbreFiscalDigital timbreFiscal, CartaPorte cartaPorte, Concepto concepto) throws Exception {
        detalleNo++;
        DecimalFormat df = new DecimalFormat("$#,##0.00");
        DecimalFormat dfI = new DecimalFormat("#0.000");
        ...
        addChunk(detalleParagraph, F.f(nullText(concepto.getNoIdentificacion()), 20, (short) (F.LJ+F.TR)), 0f);
        addChunk(detalleParagraph, F.f(nullText(concepto.getClaveUnidad().toString()), 10, (short) (F.LJ+F.TR)), 0f);
        addChunk(detalleParagraph, F.f(tokensDescripcion[0], descripcionLongitudMaxima, (short) (F.LJ+F.TR)), 0f);
        addChunk(detalleParagraph, F.f(dfI.format(concepto.getCantidad().doubleValue()), 9, (short) (F.RJ+F.TR)), 0f);
        addChunk(detalleParagraph, F.f(df.format(concepto.getValorUnitario().doubleValue()), 12, (short) (F.RJ+F.TR)), 0f);
        addChunk(detalleParagraph, F.f(df.format(concepto.getImporte().doubleValue()), 15, (short) (F.RJ+F.TR)), detalleBold, 0f);
        addChunk(detalleParagraph, F.f(df.format(concepto.getDescuento() == null ? 0.0
                : concepto.getDescuento().doubleValue()), 14, (short) (F.RJ+F.TR)), detalleBold, 0f);
        ...
        addChunk(detalleParagraph, F.f(concepto.getClaveProdServ(), 20, (short) (F.LJ+F.TR)), 0f);
        addChunk(detalleParagraph, F.f(nullText(concepto.getUnidad()), 30, (short) (F.LJ+F.TR)), 0f);
        addChunk(detalleParagraph, F.f(" ", descripcionLongitudMaxima - 20, (short) (F.LJ+F.TR)), 0f);

        detalleParagraph.add(Chunk.NEWLINE);

        if (concepto.getImpuestos()!=null && concepto.getImpuestos().getTraslados()!=null) {
            Concepto.Impuestos.Traslados.Traslado traslados[] = concepto.getImpuestos().getTraslados().getTrasladoArray();
            if (traslados.length > 0) {
                Concepto.Impuestos.Traslados.Traslado trasladoIEPS = null;
                Concepto.Impuestos.Traslados.Traslado trasladoIVA = null;
                for (Concepto.Impuestos.Traslados.Traslado traslado : traslados) {
                    // IEPS
                    if (traslado.getImpuesto().toString().compareTo("003")==0) {
                        trasladoIEPS = traslado;
                    }
                    // IVA
                    if (traslado.getImpuesto().toString().compareTo("002")==0) {
                        trasladoIVA = traslado;
                    }
                }
                if (trasladoIEPS!=null) {
                    addChunk(detalleParagraph, "Traslado", detalleBold, 0f);
                    addChunk(detalleParagraph,
                            "  Impuesto: "+trasladoIEPS.getImpuesto()+" - IEPS"
                           +"  Tipo o factor: "+trasladoIEPS.getTipoFactor()
                           +"  Tasa o cuota: "+trasladoIEPS.getTasaOCuota()
                           +"  Base: "+df.format(trasladoIEPS.getBase().doubleValue())
                           +"  Importe: "+df.format(trasladoIEPS.getImporte().doubleValue()));
                }
                if (trasladoIVA!=null) {
                    addChunk(detalleParagraph, "Traslado", detalleBold, 0f);
                    addChunk(detalleParagraph,
                            "  Impuesto: "+trasladoIVA.getImpuesto()+" - IVA"
                           +"  Tipo o factor: "+trasladoIVA.getTipoFactor()
                           +"  Tasa o cuota: "+trasladoIVA.getTasaOCuota()
                           +"  Base: "+df.format(trasladoIVA.getBase().doubleValue())
                           +"  Importe: "+df.format(trasladoIVA.getImporte().doubleValue()));
                }
            }
        }
